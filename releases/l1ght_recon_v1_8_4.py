...
def print_flow():
    ...
    lines = [
        ...
        "   " + command(
            "nmap -sU --open [portas UDP prioritárias] -Pn -n TARGET -T4"
        ),
        "",
...
def print_udp_results(udp_ports, use_section=True):
    ...
    if not udp_ports:
        print("Nenhuma porta UDP aberta identificada.")
        return
...
def run_katana(service, service_dir, depth, cookie=None, seed_urls=None):
    ...
    stdout, _, _ = run_command(
        command,
        activity=None,
        heartbeat=drain_ffuf_partial_blocks,
...
def run_ffuf_content(
    ...
    workers = 1 if LOW_NOISE else min(3, max(1, len(fuzz_bases)))
    with ThreadPoolExecutor(max_workers=workers) as ffuf_executor:
        futures = {
            ffuf_executor.submit(
                execute_base, base_url, index, True, ""
            ): (index, base_url)
    ...
    if not combined and fuzz_bases:
        ...
        workers = 1 if LOW_NOISE else min(3, max(1, len(fallback_bases)))
        with ThreadPoolExecutor(max_workers=workers) as ffuf_executor:
            futures = {
                ffuf_executor.submit(
                    execute_base, base_url, index, False, "_fallback"
                ): (index, base_url)
            ...
            for future in as_completed(futures):
                try:
                    ...
                    index, base_url = futures[future]
                    warning(f"FFUF fallback falhou no caminho {base_url}: {exc}")
                    _debug_log("FFUF_EXCEPTION", f"fallback base={base_url} index={index} exc={exc!r}")
...
def analyze_source(service, candidate_urls, ffuf_results, limit=25, cookie=None):
    ...
    session.headers.update(
        {
            "User-Agent": (
                ...
                "AppleWebKit/537.36 L1ght-Recon/1.8.3"
            )
        }
    )
    ...
    fetched.sort(key=lambda pair: (pair[0] != service["url"], pair[0]))
...
def print_final_compilation(
    ...
    print_final_port_table("PORTAS UDP", udp_ports)
...
"""
    ...
    # Remove .bak deixados por versões anteriores do updater.
    ...
    if udp_future is not None:
        try:
            udp_ports = wait_future_with_progress(udp_future, "Scan UDP")
        except Exception as exc:
